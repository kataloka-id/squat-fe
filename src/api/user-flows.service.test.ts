...
describe('UserFlowsService', () => {
  ...
  it('normalizes nullable and invalid flow metadata before list, detail, and graph rendering', () => {
    const normalized = normalizeUserFlow({ id: 'f1', flowKey: 'UF-1', title: 'Checkout', priority: null as never, health: null as never, status: null as never, linkedTestCaseCount: null as never, automatedTestCaseCount: undefined as never, coverage: Number.NaN, latestTestRun: null });
    expect(normalized).toMatchObject({ priority: 'not_defined', health: 'unknown', status: 'draft', linkedTestCaseCount: 0, automatedTestCaseCount: 0, coverage: null });
  });
});
