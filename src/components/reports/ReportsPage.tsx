...
const resultChipStyles: Record<string, string> = {
  passed: 'border-emerald-200 bg-emerald-50 text-emerald-700',
  failed: 'border-red-200 bg-red-50 text-red-700',
  blocked: 'border-amber-200 bg-amber-50 text-amber-700',
  skipped: 'border-slate-200 bg-slate-50 text-slate-600',
  untested: 'border-slate-200 bg-slate-100 text-slate-600',
};
const ResultChip = ({ result }: { result: string }) => {
  ...
  return (
    <span
      ...
      className={`inline-flex shrink-0 items-center rounded-full border px-2.5 py-1 text-xs font-semibold leading-none ${resultChipStyles[result.toLowerCase()] ?? 'border-slate-200 bg-slate-50 text-slate-600'}`}
    >
      {label}
    </span>
  );
    ...
    <div className="max-h-[calc(100dvh-8rem)] overflow-y-auto p-4 sm:p-6">
      {loading ? <p className="text-sm text-slate-500">Memuat detail hasil eksekusi…</p> : error ? <p role="alert" className="rounded-lg bg-red-50 p-4 text-sm text-red-700">{error}</p> : execution ? <div className="space-y-6 text-sm">
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <div><p className="text-sm text-slate-500">Result</p><div className="mt-2"><ResultChip result={execution.result} /></div></div>
        ...
        <div><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Step results</p><div className="mt-2 space-y-3">{execution.steps?.length ? execution.steps.map((step, index) => <div key={step.id} className="rounded-lg border border-slate-200 p-4"><div className="flex items-start justify-between gap-4"><div className="min-w-0 text-sm font-medium leading-6 text-slate-800"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Step {index + 1}</p><MarkdownContent className="mt-1 min-w-0" value={step.action} /></div><ResultChip result={step.result} /></div><div className="mt-2 text-xs leading-5 text-slate-500"><p className="font-medium text-slate-600">Expected:</p><MarkdownContent className="mt-1 min-w-0" value={step.expectedResult} /></div>{step.notes && <div className="mt-2 text-sm leading-6 text-slate-700"><span className="font-medium">Note:</span><MarkdownContent className="mt-1 text-sm leading-6" value={step.notes} /></div>}</div>) : <p className="text-slate-500">Tidak ada detail langkah.</p>}</div></div>
