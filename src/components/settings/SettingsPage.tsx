...
import { AlertCircle, Eye, EyeOff, Pencil, Plus, RefreshCw, Save, Shield, Trash2, UserCog, Users, X } from 'lucide-react';
...
const Field = ({ label, showRequiredIndicator = false, ...props }: { label: string; showRequiredIndicator?: boolean } & InputHTMLAttributes<HTMLInputElement>) => (
  <label className="block text-sm font-medium text-slate-700">
    {label}{showRequiredIndicator && <><span aria-hidden="true" data-required-marker className="ml-1 text-red-600">*</span><span className="sr-only"> (wajib)</span></>}
    <input
      {...props}
...
const PasswordField = ({ visible, onVisibilityChange, ...props }: InputHTMLAttributes<HTMLInputElement> & { visible: boolean; onVisibilityChange: () => void }) => (
  <label className="block text-sm font-medium text-slate-700">
    ...
    <span className="relative mt-1.5 block">
      ...
      />
      <button type="button" aria-label={visible ? 'Sembunyikan kata sandi baru' : 'Tampilkan kata sandi baru'} aria-pressed={visible} onClick={onVisibilityChange} className="absolute inset-y-0 right-0 flex w-10 items-center justify-center rounded-r-lg text-slate-500 transition hover:text-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-inset">
...
export const SettingsPage = () => {
  ...
  const editableRoles = editingUser?.roleSlug.toLowerCase() === 'kataloka_admin'
    ? [
      ...roles.filter((role) => role.slug.toLowerCase() === 'kataloka_admin'),
      ...assignableRoles,
    ]
    : assignableRoles;
  const roleLabel = (role: RoleRecord) => role.slug.toLowerCase() === 'kataloka_admin'
  ...
  return <div className="mx-auto w-full max-w-6xl space-y-6 animate-in fade-in duration-300">
    ...
    {loading ? <div className="rounded-xl border border-slate-200 bg-white p-8 text-sm text-slate-500">Memuat pengaturan…</div> : <>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><div className="mb-5 flex items-center gap-3"><div className="rounded-lg bg-brand-50 p-2 text-brand-600"><UserCog size={20} /></div><div><h2 className="font-semibold text-slate-900">Profil saya</h2><p className="text-sm text-slate-500">Email dan username Anda tidak dapat diubah. Anda dapat memperbarui kata sandi.</p></div></div>
        <form onSubmit={saveProfile} className="grid max-w-2xl gap-4 sm:grid-cols-2"><Field label="Email" type="email" required autoComplete="email" disabled value={profile.email} onChange={(e) => setProfile({ ...profile, email: e.target.value })} /><Field label="Username" required autoComplete="username" disabled value={profile.username} onChange={(e) => setProfile({ ...profile, username: e.target.value })} /><div className="sm:col-span-2"><PasswordField visible={showProfilePassword} onVisibilityChange={() => setShowProfilePassword((current) => !current)} minLength={8} autoComplete="new-password" value={profile.password} onChange={(e) => setProfile({ ...profile, password: e.target.value })} /></div><div className="sm:col-span-2"><Button type="submit" disabled={saving} icon={<Save size={16} />}>Simpan profil</Button></div></form>
      </section>
      {isKatalokaAdmin && <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><div className="mb-5"><h2 className="font-semibold text-slate-900">Company Management</h2><p className="text-sm text-slate-500">Buat company baru dan arsipkan company yang tidak aktif. Profil existing tidak dapat diedit di area ini.</p></div><form onSubmit={createCompany} className="mb-5 grid gap-3 rounded-lg bg-slate-50 p-4 md:grid-cols-2"><p className="md:col-span-2 text-sm text-slate-600">Field yang ditandai tanda bintang wajib diisi.</p><Field label="Company name" required showRequiredIndicator value={newCompany.name} onChange={(e) => setNewCompany({ ...newCompany, name: e.target.value })}/><Field label="Address" required showRequiredIndicator value={newCompany.address} onChange={(e) => setNewCompany({ ...newCompany, address: e.target.value })}/><Field label="Phone" required showRequiredIndicator value={newCompany.phone} onChange={(e) => setNewCompany({ ...newCompany, phone: e.target.value })}/><Field label="Email (opsional)" type="email" value={newCompany.email} onChange={(e) => setNewCompany({ ...newCompany, email: e.target.value })}/><Field label="Business field" required showRequiredIndicator value={newCompany.field} onChange={(e) => setNewCompany({ ...newCompany, field: e.target.value })}/><Field label="Postal code" required showRequiredIndicator value={newCompany.postalCode} onChange={(e) => setNewCompany({ ...newCompany, postalCode: e.target.value })}/><label className="block text-sm font-medium text-slate-700">Business type<span aria-hidden="true" data-required-marker className="ml-1 text-red-600">*</span><span className="sr-only"> (wajib)</span><select aria-label="Business type (wajib)" required value={newCompany.businessType} onChange={(e) => setNewCompany({ ...newCompany, businessType: e.target.value })} className="mt-1.5 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm"><option value="">Pilih tipe</option>{activeTypes.map((type) => <option key={type.id} value={type.id}>{type.businessType}</option>)}</select></label><label className="block text-sm font-medium text-slate-700">Category<span aria-hidden="true" data-required-marker className="ml-1 text-red-600">*</span><span className="sr-only"> (wajib)</span><select aria-label="Category (wajib)" required value={newCompany.category} onChange={(e) => setNewCompany({ ...newCompany, category: e.target.value })} className="mt-1.5 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm"><option value="">Pilih kategori</option>{activeCategories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label><div className="md:col-span-2"><Button type="submit" disabled={saving} icon={<Plus size={16}/>}>Buat company</Button></div></form><ul className="divide-y rounded-lg border border-slate-200">{managedCompanies.map((managedCompany) => <li key={managedCompany.id} className="flex items-center justify-between gap-3 p-3"><div><p className="font-medium text-slate-800">{managedCompany.name}</p><p className="text-xs text-slate-500">{managedCompany.isActive ? 'Aktif' : 'Diarsipkan'}</p></div><Button variant={managedCompany.isActive ? 'danger' : 'secondary'} size="sm" disabled={saving} onClick={() => void updateCompanyStatus(managedCompany)}>{managedCompany.isActive ? 'Arsipkan' : 'Aktifkan'}</Button></li>)}</ul></section>}
    ...
    {isCompanyAdmin && assignmentUser && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" role="dialog" aria-modal="true" aria-labelledby="assignment-title">
