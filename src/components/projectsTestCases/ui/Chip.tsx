...
export type ChipType = 'priority' | 'status' | 'tag' | 'automation' | 'automationReadiness' | 'testRunType' | 'assignee' | 'section' | 'health' | 'generic';
...
const tones: Record<ChipType, Record<string, string>> = {
  ...
  testRunType: { Manual: 'bg-slate-100 text-slate-600 border-slate-200', Automated: 'bg-cyan-50 text-cyan-700 border-cyan-200', Mixed: 'bg-violet-50 text-violet-700 border-violet-200' },
  assignee: { _: 'bg-slate-50 text-slate-700 border-slate-200' },
...
const dotColors: Record<ChipType, Record<string, string>> = {
  ...
  testRunType: { Manual: 'bg-slate-400', Automated: 'bg-cyan-500', Mixed: 'bg-violet-500' },
  assignee: {},
...
export const Chip = ({ type = 'generic', value, label, displayValue, onClick, disabled = false, className = '', leadingIcon }: ChipProps) => {
  ...
  const tone = tones[type][value] ?? tones[type]._ ?? (type === 'section' ? 'bg-slate-50 text-slate-600 border-slate-200' : 'bg-slate-100 text-slate-600 border-slate-200');
