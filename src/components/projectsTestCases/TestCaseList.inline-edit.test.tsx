...
const testCase: TestCase = {
  id: 'tc-1', tcNumber: 1, projectId: 'project-1', projectKey: 'PRJ', title: 'Example case', section: 'General',
...
const renderList = (onUpdate: (updates: Partial<TestCase>) => void | Promise<void>, canManage = true) => render(
  <TestCaseList
    testCases={[testCase]}
    projects={[]}
    selectedIds={[]}
...
const UpdatingList = () => {
  ...
  return <TestCaseList testCases={[current]} projects={[]} selectedIds={[]} sortField="id" sortOrder="asc" onSort={() => {}} onToggleSelect={() => {}} onToggleSelectAll={() => {}} onEdit={() => {}} onDelete={() => {}} onUpdate={async (_id, updates) => setCurrent((value) => ({ ...value, ...updates }))} hasProjectSelected canManage />;
};
