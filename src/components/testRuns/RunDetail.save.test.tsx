...
const second = {
  ...untested,
  id: 'execution-2',
  sourceTestCaseId: 'case-2',
  snapshot: { tcNumber: 2, title: 'Second case', steps: [] },
};
const third = {
  ...untested,
  id: 'execution-3',
  sourceTestCaseId: 'case-3',
...
describe('RunDetail execution save', () => {
  ...
  it('replaces redundant detail/finish menu entries with edit/delete and confirms delete', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Edit Test Cases' })).toBeTruthy();
  ...
  it('hides edit and finish actions for completed runs while retaining delete', async () => {
    serviceMocks.get.mockResolvedValue({
      data: {
        ...runResponse([{ ...untested, result: 'Passed' as const }]).data,
        status: 'Completed',
      },
    });
    const user = userEvent.setup();
  ...
  it('renders Test Run header metadata as separate chips', async () => {
    ...
    render(
      <RunDetail
        projectId="project-1"
        runId="run-1"
        currentUser={{ id: 'session-1', username: 'logged_in_user' }}
        onBack={vi.fn()}
      />,
    );
    ...
    expect(screen.queryByRole('button', { name: 'Selesaikan Run' })).toBeNull();
    expect(screen.getByLabelText('Deskripsi Test Run').textContent).toContain(
      'Run smoke untuk checkout.',
    );
  });
  ...
  it('keeps assignee and User Flow metadata out of each compact list item', async () => {
    ...
    expect(screen.getByLabelText('Test case metadata').textContent).toContain(
      'UF-1 · Sign in flow',
    );
    expect(screen.getByLabelText('Assignee')).toBeTruthy();
  });

  it('uses the same canonical project-key Test Case ID in the list and detail', async () => {
    serviceMocks.get.mockResolvedValue(
      runResponse([
        {
          ...untested,
          snapshot: { projectKey: 'PAY', tcNumber: 11, title: 'Checkout', steps: [] },
        },
      ]),
    );
    ...
    expect(
      screen.getByRole('heading', { name: 'Test cases in run' }).closest('aside')?.textContent,
    ).toContain('PAY-11');
    expect(screen.getByLabelText('Test case metadata').textContent).toContain('PAY-11');
    expect(screen.queryByText(/TC-11/)).toBeNull();
  });
  ...
  it('keeps the owner chip sourced from the Test Run, not the authenticated user', async () => {
    ...
    const { rerender } = render(
      <RunDetail
        projectId="project-1"
        runId="run-1"
        currentUser={{ id: 'session-1', username: 'first_user' }}
        onBack={vi.fn()}
      />,
    );
    ...
    rerender(
      <RunDetail
        projectId="project-1"
        runId="run-1"
        currentUser={{ id: 'session-2', username: 'second_user' }}
        onBack={vi.fn()}
      />,
    );
  ...
  it('refetches server-derived completed status and summary after a successful result save', async () => {
    ...
    await waitFor(() =>
      expect(serviceMocks.updateExecution).toHaveBeenCalledWith(
        'project-1',
        'run-1',
        'execution-1',
        { result: 'Passed', notes: '' },
  ...
  it('keeps Save & Next available for a middle case even when the sidebar is filtered', async () => {
    ...
    const user = userEvent.setup();
    render(
      <RunDetail projectId="project-1" runId="run-1" executionId="execution-2" onBack={vi.fn()} />,
    );
  ...
  it('shows the completed state automatically for a fully executed case', async () => {
    ...
    serviceMocks.get.mockResolvedValue({
      data: { ...runResponse([completedCase]).data, status: 'Completed' },
    });
    render(<RunDetail projectId="project-1" runId="run-1" onBack={vi.fn()} />);
  ...
  it('does not offer silent completion while any case remains Untested', async () => {
    serviceMocks.get.mockResolvedValue(
      runResponse([untested, { ...second, result: 'Passed' as const }]),
    );
    render(
      <RunDetail projectId="project-1" runId="run-1" executionId="execution-2" onBack={vi.fn()} />,
    );
